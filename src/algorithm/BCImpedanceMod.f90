MODULE BCImpedanceMod
  ...
  SUBROUTINE BCImpedance( x, BotTop, HS, f, g, iPower, ComplexFlag )
    ...
    ! Shoot through elastic layers   对弹性层做处理
    SELECT CASE ( BotTop )
    CASE ( 'TOP' )
       IF ( FirstAcoustic > 1 ) THEN   ! Shoot down from top   如果声学层上方存在弹性层，
       ...
       END IF
    CASE ( 'BOT' )
       IF ( LastAcoustic < SSP%NMedia ) THEN   ! Shoot up from bottom   如果声学层下方存在弹性层
