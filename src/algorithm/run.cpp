...
void run()
{
    ...
    for (size_t iprof = 0; iprof < params.NProf; iprof++)
    {
        ...
        for (int i = 0; i < params.SSP[iprof].NMedia; ++i)
        {
            ...
            {
                // 打印警告信息（中文）
                cout << "警告：KRAKEN 垂直网格步长太大，已经将网格数量: " << params.SSP[iprof].NMesh[i] << " 调整为: " << Nneeded << endl;
                params.SSP[iprof].NMesh[i] = Nneeded;
            }
        ...
        // 初始化cp cs
        UpdateSSPLoss(params.freqinfo->freq, params.freqinfo->freq, params.SSP[iprof].NMedia,
                      params.SSP[iprof].SSPType, params.AttenUnit, params.SSP[iprof]);
        UpdateHSLoss(params.freqinfo->freq, params.freqinfo->freq,
                     params.AttenUnit, params.HSTop[iprof], params.HSBot[iprof]);
        ...
        int M = 0;
        VectorXd Ex1 = eigen[iprof].Extrap.segment(0, eigen[iprof].M);
        // cout << "Ex1: \n"
        //      << Ex1 << endl;
        while (M < eigen[iprof].M && Ex1(M) > SQ(2 * pi * params.freqinfo->freq / params.Chigh))
        {
            M++;
        }
        eigen[iprof].M = M;
        // 使用tempk存储本征值
        VectorXcd tempk = eigen[iprof].k.segment(0, M);
