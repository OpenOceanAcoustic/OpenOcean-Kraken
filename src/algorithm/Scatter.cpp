...
std::complex<double> KupIng(double sigma,
    ...
    // Del非零时计算散射扰动
    if (std::abs(Del) > 1e-15) {  // 避免除以零，用极小值判断
        std::complex<double> A11, A12, A21, A22;

        // 计算A11：对应原Fortran公式
        A11 = 0.5 * (eta1Sq - eta2Sq) 
              - (rho2 * eta1Sq - rho1 * eta2Sq) * (eta1 + eta2) / Del;

        // 计算A12：含复数单位i的项
        A12 = I1D * std::pow(rho2 - rho1, 2) * eta1 * eta2 / Del;

        // 计算A21：含负i的项
        A21 = -I1D * std::pow(rho2 * eta1Sq - rho1 * eta2Sq, 2) 
              / (rho1 * rho2 * Del);

        // 计算A22：对应原Fortran公式
        A22 = 0.5 * (eta1Sq - eta2Sq) 
              + (rho2 - rho1) * eta1 * eta2 * (eta1 + eta2) / Del;

        // 计算最终散射扰动结果（原Fortran的KupIng）
        result = -std::pow(sigma, 2) 
                 * (-A21 * P * P + (A11 - A22) * P * U + A12 * U * U);
    }
